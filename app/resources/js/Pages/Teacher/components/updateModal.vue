...
function open(props: { data: FormTypes}) {
    ...
    _props.sections = _props.sections?.map(i => i?.id) as []
...
function submit() {
    ...
    form.patch(route('teachers.update'), {
        onSuccess: () => {
            toast.add({ severity: 'success', summary: 'Success', detail: 'Updated Teacher Successfully', life: 3000 });
            close()
            form.reset()
            router.reload()
        },
